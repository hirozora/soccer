...
BACKGROUND="$ROOT/experiments/task_view_fusion_v1/background"
UNIT="football-hgt-targets-v4-task-view-fusion"
mkdir -p "$BACKGROUND"

if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "Task-view fusion study is already running as $UNIT.service"
  exit 0
fi
...
systemd-run --user --unit "$UNIT" --collect \
  ...
  "$PYTHON" "$ROOT/scripts/run_task_view_fusion_matrix.py" \
