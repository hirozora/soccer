...
BACKGROUND="$ROOT/experiments/five_task_view_v1/background"
UNIT="football-hgt-targets-v4-five-task-view"
mkdir -p "$BACKGROUND"

if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "Five-task view study is already running as $UNIT.service"
  exit 0
fi
...
systemd-run --user --unit "$UNIT" --collect \
  ...
  "$PYTHON" "$ROOT/scripts/run_five_task_view_matrix.py" \
