#!/usr/bin/env python
"""Resumable task-specific age-aware readout pipeline."""
...
from football_hgt_targets_v4.age_pooling_reporting import build_report, lock_validation  # noqa: E402
from football_hgt_targets_v4.age_pooling_study import AGE_POOL_ROOT, TRAINED, checkpoint_path, lock_path, test_dir, training_dir  # noqa: E402
from football_hgt_targets_v4.constants import CONFIRMATION_SEEDS  # noqa: E402

RUNNER = ROOT / "scripts/run_age_pooling_experiment.py"
...
def _jobs(stage: str, devices: list[str], *, smoke: bool = False, test: bool = False) -> list[Job]:
    ...
    for index, (configuration, seed) in enumerate((c, s) for c in configurations for s in seeds):
        output = AGE_POOL_ROOT / "smoke" / configuration / f"seed{seed}" if smoke else test_dir(configuration, seed) if test else training_dir(configuration, seed)
        command = [
...
def execute(jobs: list[Job], workers: int, force: bool, *, allow_ineligible: bool = False) -> None:
    status_path = AGE_POOL_ROOT / "background/status.json"
...
def verify() -> None:
    subprocess.run([sys.executable, "-m", "pytest", "tests/test_age_pooling.py"], cwd=ROOT, check=True)
    validate_age_index_contract(AGE_POOL_ROOT / "validation/age_index_contract.json")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--stage", choices=("verify", "smoke", "validation", "lock", "test", "efficiency", "summarize", "pipeline"), required=True)
    ...
    if args.stage == "smoke": execute(_jobs("smoke", args.devices, smoke=True), min(2, len(args.devices)), args.force); return
    ...
    if args.stage == "efficiency": print(benchmark_age_pooling(f"cuda:{args.devices[0]}")); return
    if args.stage == "summarize": print(build_report()); return
    verify()
    execute(_jobs("smoke", args.devices, smoke=True), min(2, len(args.devices)), args.force)
    ...
    benchmark_age_pooling(f"cuda:{args.devices[0]}")
    print(build_report())
