#!/usr/bin/env python
"""Resumable strict task-receptive-field pipeline."""
...
from football_hgt_targets_v4.receptive_field_reporting import build_report, lock_validation  # noqa: E402
from football_hgt_targets_v4.receptive_field_efficiency import benchmark_receptive_fields  # noqa: E402
from football_hgt_targets_v4.receptive_field_study import RF_ROOT, TRAINED, checkpoint_path, lock_path, test_dir, training_dir  # noqa: E402

RUNNER = ROOT / "scripts/run_receptive_field_experiment.py"
...
def _jobs(stage: str, devices: list[str], *, smoke: bool = False, test: bool = False) -> list[Job]:
    ...
    for index, (configuration, seed) in enumerate((c, s) for c in configurations for s in seeds):
        output = RF_ROOT / "smoke" / configuration / f"seed{seed}" if smoke else test_dir(configuration, seed) if test else training_dir(configuration, seed)
        command = [
        ...
        jobs.append(Job(f"{stage}/{configuration}/seed{seed}", tuple(command), output / "result.json", output / f"console_{stage}.log"))
    return jobs


def execute(jobs: list[Job], workers: int, force: bool, *, allow_ineligible: bool = False) -> None:
    status_path = RF_ROOT / "background/status.json"
    ...
    def run(job: Job) -> None:
        ...
        if completed.returncode and allow_ineligible:
            ...
            if len(rows) == 24 and not any(row.get("guarded_core_eligible") for row in rows):
...
def verify() -> None:
    subprocess.run([sys.executable, "-m", "pytest", "tests/test_receptive_field.py"], cwd=ROOT, check=True)
...
def main() -> None:
    ...
    if args.stage == "efficiency": print(benchmark_receptive_fields(f"cuda:{args.devices[0]}")); return
    ...
    benchmark_receptive_fields(f"cuda:{args.devices[0]}")
    print(build_report())
