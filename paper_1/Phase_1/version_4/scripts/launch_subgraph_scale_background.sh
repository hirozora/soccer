...
BACKGROUND="$ROOT/experiments/subgraph_scale_v1/background"
UNIT="football-hgt-targets-v4-subgraph-scale"
mkdir -p "$BACKGROUND"

if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "Subgraph scale study is already running as $UNIT.service"
  exit 0
fi
...
systemd-run --user --unit "$UNIT" --collect \
  ...
  "$PYTHON" "$ROOT/scripts/run_subgraph_matrix.py" \
