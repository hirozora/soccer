...
BACKGROUND="$ROOT/experiments/state_aware_relation_gating_v1/background"
UNIT="football-hgt-v4-state-gating"
mkdir -p "$BACKGROUND"
if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "State-gating study is already running as $UNIT.service"
  exit 0
fi
...
systemd-run --user --unit "$UNIT" --collect \
  ...
  "$PYTHON" "$ROOT/scripts/run_state_gating_matrix.py" \
