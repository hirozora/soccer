...
BACKGROUND="$ROOT/experiments/layerwise_partial_sharing_v1/background"
UNIT="football-hgt-targets-v4-partial-l2"
mkdir -p "$BACKGROUND"
if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "Partial-L2 study is already running as $UNIT.service"
  exit 0
fi
...
systemd-run --user --unit "$UNIT" --collect \
  ...
  "$PYTHON" "$ROOT/scripts/run_partial_sharing_matrix.py" \
    --stage pipeline --devices 0 1 2 3 >/dev/null
