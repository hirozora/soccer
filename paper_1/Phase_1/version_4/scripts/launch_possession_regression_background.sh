...
BACKGROUND="$ROOT/experiments/possession_regression/background"
UNIT="football-hgt-targets-v4-possession"
mkdir -p "$BACKGROUND"

if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "Possession regression is already running as $UNIT.service"
  exit 0
fi
...
systemd-run --user --unit "$UNIT" --collect \
  ...
  "$PYTHON" "$ROOT/scripts/run_possession_matrix.py" \
