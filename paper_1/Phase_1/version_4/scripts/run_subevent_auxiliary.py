#!/usr/bin/env python
"""Independent Rotate-based Subevent auxiliary pipeline, with validation-only selection."""
import argparse
import json
import os
...
from football_hgt_targets_v4.subevent_training import ROOT, SEEDS, sources, train_subevent, refit_subevent, write_json
from football_hgt_targets_v4.subevent_verification import audit, smoke, diagnostics
from football_hgt_targets_v4.subevent_reporting import selection, test_locked, report
...
def verify(root):
    ...
    with (out / 'unit_tests.log').open('w') as handle:
        result = subprocess.run([sys.executable, '-m', 'pytest', 'tests/test_subevent_auxiliary.py', '-q'],
...
def main():
    ...
    parser.add_argument('--mode', choices=('coarse', 'fine'))
    ...
    if args.seed is not None or args.mode is not None:
        ...
        if args.stage == 'train':
            train_subevent(args.mode, args.seed, args.device, root, resume_from=args.resume)
        else:
            refit_subevent(args.mode, args.seed, args.device, root)
        return
    if args.resume:
    ...
    try:
        ...
        tasks = [(m, s) for m in ('coarse', 'fine') for s in SEEDS]
        if args.stage in ('train', 'pipeline'):
            require_verified(root)
            if not (root / 'selection/subevent_auxiliary_lock.json').exists():
