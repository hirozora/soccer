...
BACKGROUND="$ROOT/experiments/pipeline/background"
UNIT="football-hgt-targets-v4-light-pipeline"
mkdir -p "$BACKGROUND"

if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "Light confirmation pipeline is already running as $UNIT.service"
  exit 0
fi
...
echo "Started light confirmation pipeline as $UNIT.service with PID $PID"
