...
BACKGROUND="$ROOT/experiments/screen/background"
UNIT="football-hgt-targets-v4-screen"
mkdir -p "$BACKGROUND"

if systemctl --user is-active --quiet "$UNIT.service"; then
  echo "Screen matrix is already running as $UNIT.service"
  exit 0
fi
...
systemd-run --user --unit "$UNIT" --collect \
  ...
  "$PYTHON" "$ROOT/scripts/run_matrix.py" \
    --stage screen --devices 0 1 2 3 --slots-per-gpu 3 >/dev/null
...
echo "Started Version 4 screen matrix as $UNIT.service with PID $PID"
