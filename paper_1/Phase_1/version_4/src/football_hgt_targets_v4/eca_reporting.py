"""Validation-only ECA selection, paired match inference and confusion audit."""
import json
...
from .eca_training import (ROOT, ROTATE_ROOT, SEEDS, run_dir, frame_path, require_test,
...
def choose(comparisons, losses, complete):
    ...
    selected = next(m for m in ('base', 'constant', 'transition')
...
def selection(root=ROOT):
    ...
    for mode in ('constant', 'transition'):
        ...
        if any(r['epochs'] != 24 or r['steps'] != 3192 for r in results):
            raise RuntimeError('Incomplete ECA budget')
    ...
    comparisons = {f'{a}-{b}': compare(frames[b], frames[a]) for a, b in
                   (('constant', 'base'), ('transition', 'constant'), ('transition', 'base'))
    ...
    test_models = [] if selected == 'base' else (['base', 'constant', 'transition']
                                               if selected == 'transition' else ['base', 'constant'])
...
def report(root=ROOT):
    lock = json.loads((root / 'selection/eca_lock.json').read_text())
    rows, raw_rows, groups = [], [], []
    output = root / 'reports'
    ...
    for split, modes in (('validation', lock['complete']), ('test', lock['test_models'])):
        ...
        if split == 'test' and modes:
            write_json(output / 'test_bootstrap.json', {f'{a}-{b}': compare(frames[b], frames[a])
                for a, b in (('constant', 'base'), ('transition', 'constant'), ('transition', 'base'))
    ...
    (output / 'README.md').write_text(
        '# Main-L2 Possession-transition ECA\n\n'
        ...
        'Constant improvements alone do not establish conditional edge value.\n'
        'Overall improvements do not establish the target confusion mechanism without a matching confusion reduction.\n'
        'No claim about hub mixing or task-specific receptive fields; no automatic ECA expansion.\n')
